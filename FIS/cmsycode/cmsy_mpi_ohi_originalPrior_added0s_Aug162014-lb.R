##------------------------------------------
## MPI wrapper on C_MSY_MARK.R
## CM: Fri June 28 2013
## Notes: 
##------------------------------------------
#library(Rmpi)#lb

##setwd('/Volumes/LACIE SHARE/LAPUBLIC/SAUP_Projects/FishExpStatusWG/SingleStatus/Mark_Stocks/')# Put in your directory
#cdat<-read.csv("OHICatchHistoryCMSY_added0s_07_21_2014.csv", stringsAsFactors=FALSE)
cdat<-read.csv("MSYmatrix_categoricalRes2.csv",stringsAsFactors=FALSE )
##head(cdat)
...
##-------------
## CM
## see: http://math.acadiau.ca/ACMMaC/Rmpi
## change the number of slaves 
#nslaves<-1913 ##length(input)
#mpi.spawn.Rslaves(nslaves=nslaves)

## MPI HOUSEKEEPING
#if (mpi.comm.size() != nslaves+1) {
#    print("Please initialize an MPI cluster of at least nslaves+1 processors.")
...
## Function called on each processor to perform Catch-MSY assessment
## with stock number equal to their processor number
## Assumes: input, stockNumber

runCMSY<-function(){
  stock_id<-unique(cdat$stock)
  ##
  ...
  meanct<-(ct[length(ct)-4]+ct[length(ct)-3]+ct[length(ct)-2]+ct[length(ct)])/5 #calculate mean over last 5 years  
  ## PARAMETER SECTION
  # R prior
  ...
  finalbio <- if(ct[nyr]/max(ct) > 0.5) {c(0.3,0.7)} else {c(0.01,0.4)} ## use for batch processing   ****FROM RAINER's VERSION--Oct 12, 2012
  ...
  if(length(r1)>=10) {
    ...
    ##R6<-aggregate(log(B_Bmsy)~as.numeric(Count)+Stock, data=R5, mean) # WHY NOT GETTING THE QUANTILES DIRECTLY FROM R5?
        R6<-aggregate(log(B_Bmsy)~as.numeric(Count)+Stock, data=R5, FUN=function(z){c(mean=mean(z),sd=sd(z),upr=exp(quantile(z, p=0.975)), lwr=exp(quantile(z, p=0.025)), lwrQ=exp(quantile(z, p=0.25)), uprQ=exp(quantile(z, p=0.75)), median=median(z))})
    ...
    ## CM: sort out columns
    R6<-data.frame(cbind(R6[,1:2],R6[,3][,1],R6[,3][,2],R6[,3][,3],R6[,3][,4],R6[,3][,5], R6[,3][,6], R6[,3][,7]))
    names(R6)<-c("Count", "Stock", "BoverBmsy", "BoverBmsySD","BoverBmsyUpper","BoverBmsyLower","BoverBmsylwrQ","BoverBmsyuprQ", "BoverBmsyMED")      ## CM: remove  last entry as it is 1 greater than number of years
    ...
    ##KK: median
    R6$Med_B_Bmsy<-exp(R6$BoverBmsyMED)
    ##
    R6<-R6[order(R6[,1]), ]
    ##BoverBmsy<-R6$GM_B_Bmsy
    ## CM: note using arithmetic mean
    BoverBmsy<-R6$M_B_Bmsy
    BoverBmsyGM<-R6$GM_B_Bmsy
    BoverBmsyMed<-R6$Med_B_Bmsy
    ...
    outputSIM = list("stock_id"=stock_id[stockNumber], "b_bmsy"=BoverBmsy, "b_bmsyUpper"=b_bmsyUpper, "b_bmsyLower"=b_bmsyLower, "b_bmsy_iq25"=b_bmsylwrQ, "b_bmsy_iq75"=b_bmsyuprQ, "b_bmsyGM"=BoverBmsyGM, "b_bmsyMed"=BoverBmsyMed, "year"=yr, "seed"=seed, "convergence"=convergence, "n_iterations"=n, "effective_sample_size"=effective_sample_size,  "method_id"=method_id, r_params=r1, k_params=k1)
  }else{
    outputSIM = list("stock_id"=stock_id[stockNumber], "b_bmsy"=NA, "b_bmsyUpper"=NA, "b_bmsyLower"=NA, "b_bmsy_iq25"=NA, "b_bmsy_iq75"=NA, "b_bmsyGM"=NA, "b_bmsyMed"=NA, "year"=yr, "seed"=seed, "convergence"="NC", "n_iterations"=n, "effective_sample_size"=effective_sample_size,  "method_id"=method_id, r_params=r1, k_params=k1)
  }
  ##
...
## setup each core
#mpi.bcast.cmd(stockNumber <- mpi.comm.rank())

## send the data to the nodes - inefficient but "input" small
#mpi.bcast.Robj2slave(cdat)

## Send run function to the children
#mpi.bcast.Robj2slave(runCMSY)

## execute the call
#system.time(cmsy.result <- mpi.remote.exec(runCMSY()))
...
## save main results list
tmp0<-lapply(cmsy.result,FUN=function(x){data.frame(
                           "stock_id"=x$stock_id,
                           "convergence"=x$convergence,
                           "effective_sample_size"=x$effective_sample_size,
                           "yr"=x$year,
                           "b_bmsy"=x$b_bmsy,
                           "b_bmsyUpper"=x$b_bmsyUpper,
                           "b_bmsyLower"=x$b_bmsyLower,
                           "b_bmsyiq25"=x$b_bmsy_iq25,
                           "b_bmsyiq75"=x$b_bmsy_iq75,
                           "b_bmsyGM"=x$b_bmsyGM,
                           "b_bmsyMed"=x$b_bmsyMed,
                           row.names = NULL)})

cmsy.ohi.orig.with0.df<-do.call(rbind,tmp0)

rownames(cmsy.ohi.orig.with0.df)<-NULL

file.string<-paste("./cmsy_ohi_results_table.RData", sep="")
save(list = c("cmsy.ohi.orig.with0.df"), file = file.string)
